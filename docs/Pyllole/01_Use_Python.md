...
Per creare un file script **basta un semplice [editor di testo](https://github.com/pythonitalia/python-abc/blob/master/docs/Pyllole/05_Development_Environment.md)**, nulla di più. Studiando Python è meglio evitare complessi ambienti di sviluppo (IDE), quando si conoscerà il linguaggio e si svilupperanno veri programmi, si potrà decidere di utilizzarli per ottimizzare il proprio lavoro.
